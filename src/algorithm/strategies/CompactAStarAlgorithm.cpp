...
int32_t CompactAStarAlgorithm::heuristic(const std::vector<compact::Item>& items) const {
    int32_t h = 0;
    const auto& target = _input->target_item.enchantments;
...
const CompactAStarAlgorithm::StepNode* CompactAStarAlgorithm::record_step(
    const StepNode* prev_tail,
    const compact::Item& base, const compact::Item& sacrifice,
    int32_t cost)
{
    return alloc_step(prev_tail, EnchSolution::EnchStep{
        compact::to_domain(base, _equipment),
        compact::to_domain(sacrifice, _equipment),
        cost,
...
void CompactAStarAlgorithm::execute(const AlgorithmInput& input, ExecutionContext& ctx) {
    ...
    // 4. Open set: min-heap ordered by f = g + h
    ...
    // 5. Closed set: state -> best known g-cost
    ...
    while (!open_set.empty() && !ctx.is_cancelled()) {
        ...
        for (size_t i = 0; i < n; ++i) {
            for (size_t j = 0; j < n; ++j) {
                ...
                // Save originals for step recording
                ...
                // Record step (converts compact → domain)
                const StepNode* step_node = record_step(
                    current.state.steps_tail,
                    base_item, sac_item, step_cost);
                ...
                // Skip if we already know a better path to this state
