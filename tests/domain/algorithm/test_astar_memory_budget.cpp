...
TEST_CASE("test_budget_from_1gb_9_items") {
    ...
    expect(b.max_step_pool >= b.max_open_set, "step_pool should have at least as many entries as open_set");
    // Items is most expensive, should have much fewer
    expect(b.max_items_pool <= b.max_step_pool, "items_pool should have fewer entries than step_pool");
}
...
TEST_CASE("test_budget_reserve_sizes") {
    auto b = AStarMemoryBudget::from_memory_mb(1024, 9);
    expect(b.reserve_step_pool > 0 && b.reserve_step_pool <= b.max_step_pool, "step_pool reserve should be <= max");
    expect(b.reserve_open_set > 0 && b.reserve_open_set <= b.max_open_set, "open_set reserve should be <= max");
}
