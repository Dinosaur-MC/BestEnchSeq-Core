...
if(MEMBER)
    ...
    # Linear-complexity body: collect per-line strings in a list, then one
    # string(JOIN) — repeated string(APPEND) into a growing string is O(n^2)
    # on CMake < 4.0 and makes 300 KB+ resources look like a hang.  32 bytes
    # per line keeps the line strings small.
    set(_lines "")
    set(_i 0)
    set(_per_line 32)
    ...
    file(WRITE "${OUTPUT}"
        ...
        "namespace besq::data {${_NL}"
        "namespace {${_NL}"
        "constexpr std::array<unsigned char, ${_data_len}> k${MEMBER} = {${_NL}${_bytes_text}${_NL}};${_NL}"
        "} // namespace${_NL}"
        "${_NL}"
        "namespace detail {${_NL}"
