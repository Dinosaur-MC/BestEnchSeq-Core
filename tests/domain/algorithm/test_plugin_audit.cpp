...
void test_audit_empty_file() {
    ...
    TempFile tf(empty);
    if (!tf.valid()) {
        TEST_PASS("audit: empty file (skipped — temp file creation failed)");
        return;
    }
...
void test_audit_truncated_binary() {
    ...
    TempFile tf(tiny);
    if (!tf.valid()) {
        TEST_PASS("audit: truncated (skipped)");
        return;
    }
    auto r = audit_plugin_binary(tf.path);
    ...
    if (!tf2.valid()) {
        TEST_PASS("audit: truncated ELF (skipped)");
        return;
    }
...
void test_audit_valid_pe() {
    ...
    TempFile tf(pe);
    if (!tf.valid()) {
        TEST_PASS("audit: valid PE (skipped)");
        return;
    }
    auto r = audit_plugin_binary(tf.path);
...
void test_audit_pe_wx_section() {
    ...
    TempFile tf(pe);
    if (!tf.valid()) {
        TEST_PASS("audit: PE W+X (skipped)");
        return;
    }
    auto r = audit_plugin_binary(tf.path);
...
void test_audit_pe_32bit_rejected() {
    ...
    TempFile tf(pe);
    if (!tf.valid()) {
        TEST_PASS("audit: PE32 rejected (skipped)");
        return;
    }
    auto r = audit_plugin_binary(tf.path);
...
void test_audit_valid_elf() {
    ...
    TempFile tf(elf);
    if (!tf.valid()) {
        TEST_PASS("audit: valid ELF (skipped)");
        return;
    }
    auto r = audit_plugin_binary(tf.path);
    expect(r.passed, "audit: minimal ELF64 passes");
    expect(r.limited, "audit: section-less ELF is marked limited (opaque)");
...
void test_audit_elf_wx_segment() {
    ...
    TempFile tf(elf);
    if (!tf.valid()) {
        TEST_PASS("audit: ELF W+X (skipped)");
        return;
    }
    auto r = audit_plugin_binary(tf.path);
...
void test_audit_elf_32bit_rejected() {
    ...
    TempFile tf(elf);
    if (!tf.valid()) {
        TEST_PASS("audit: ELF32 rejected (skipped)");
        return;
    }
    auto r = audit_plugin_binary(tf.path);
...
void test_audit_elf_safe_segment() {
    ...
    TempFile tf(elf);
    if (!tf.valid()) {
        TEST_PASS("audit: ELF safe segment (skipped)");
        return;
    }
    auto r = audit_plugin_binary(tf.path);
