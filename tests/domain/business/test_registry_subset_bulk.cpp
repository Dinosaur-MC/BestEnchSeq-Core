...
void test_subset_by_max_level() {
    ...
    infos.emplace_back(NSID("minecraft:ench_2"), "Ench 2", MCE::All, 2, 2, 1, false,
                       std::unordered_set<NSID>{}, std::unordered_set<NSID>{});
    infos.emplace_back(NSID("minecraft:ench_3"), "Ench 3", MCE::All, 3, 3, 1, false,
                       std::unordered_set<NSID>{}, std::unordered_set<NSID>{});
    infos.emplace_back(NSID("minecraft:ench_4"), "Ench 4", MCE::All, 4, 4, 1, false,
                       std::unordered_set<NSID>{}, std::unordered_set<NSID>{});

    EnchantmentRegistry reg(infos);
    auto subset = reg.create_subset(
        [](const EnchInfo& e) { return e.max_level >= 3; });
...
void test_subset_by_platform() {
    ...
    infos.emplace_back(NSID("minecraft:ench_java"), "Java", MCE::Java, 5, 5, 1, false,
                       std::unordered_set<NSID>{}, std::unordered_set<NSID>{});
    // Bedrock-only
    infos.emplace_back(NSID("minecraft:ench_bedrock"), "Bedrock", MCE::Bedrock, 5, 5, 1, false,
                       std::unordered_set<NSID>{}, std::unordered_set<NSID>{});
    // Both platforms
    infos.emplace_back(NSID("minecraft:ench_all"), "All", MCE::All, 5, 5, 1, false,
                       std::unordered_set<NSID>{}, std::unordered_set<NSID>{});
    ...
    auto subset = reg.create_subset(
        [](const EnchInfo& e) { return supports_platform(e, MCE::Java); });
...
void test_subset_by_applicable() {
    ...
    infos.emplace_back(NSID("minecraft:protection"), "Protection", MCE::All, 4, 4, 1, false,
                       std::unordered_set<NSID>{},
                       std::unordered_set<NSID>{EquipmentTag::chestplate()});
    // Unbreaking applies to both (or neither via empty set — just a control)
    infos.emplace_back(NSID("minecraft:unbreaking"), "Unbreaking", MCE::All, 3, 3, 1, false,
                       std::unordered_set<NSID>{},
                       std::unordered_set<NSID>{});
...
void test_subset_empty_result() {
    ...
    infos.emplace_back(NSID("minecraft:sharpness"), "Sharpness", MCE::All, 5, 5, 1, false,
                       std::unordered_set<NSID>{}, std::unordered_set<NSID>{});
    infos.emplace_back(NSID("minecraft:smite"), "Smite", MCE::All, 5, 5, 1, false,
                       std::unordered_set<NSID>{}, std::unordered_set<NSID>{});

    EnchantmentRegistry reg(infos);
    // Match nothing: max_level > 100
    auto subset = reg.create_subset(
        [](const EnchInfo& e) { return e.max_level > 100; });
...
void test_subset_all_match() {
    ...
    auto subset = reg.create_subset(
        [](const EnchInfo& e) { return e.max_level > 0; });
...
void test_subset_equipment() {
    ...
    eqs.emplace_back(Equipment{NSID("minecraft:iron_sword"), "Iron Sword",
                                EquipmentTag::sword(), 250});
    eqs.emplace_back(Equipment{NSID("minecraft:diamond_chestplate"), "Diamond Chestplate",
                                EquipmentTag::chestplate(), 528});
    eqs.emplace_back(Equipment{NSID("minecraft:diamond_pickaxe"), "Diamond Pickaxe",
                                EquipmentTag::pickaxe(), 1561});

    EquipmentRegistry reg(eqs);

    // Filter by sword category
    auto swords = reg.create_subset(
        [](const Equipment& e) { return e.category == EquipmentTag::sword(); });
    ...
    auto pickaxes = reg.create_subset(
        [](const Equipment& e) { return e.category == EquipmentTag::pickaxe(); });
    ...
    auto chestplates = reg.create_subset(
        [](const Equipment& e) { return e.category == EquipmentTag::chestplate(); });
...
void test_subset_chained() {
    ...
    infos.emplace_back(NSID("minecraft:ench_a"), "Ench A", MCE::Java, 5, 5, 1, false,
                       std::unordered_set<NSID>{}, std::unordered_set<NSID>{});
    // max_level=5, Bedrock
    infos.emplace_back(NSID("minecraft:ench_b"), "Ench B", MCE::Bedrock, 5, 5, 1, false,
                       std::unordered_set<NSID>{}, std::unordered_set<NSID>{});
    // max_level=2, Java
    infos.emplace_back(NSID("minecraft:ench_c"), "Ench C", MCE::Java, 2, 2, 1, false,
                       std::unordered_set<NSID>{}, std::unordered_set<NSID>{});
    // max_level=2, Bedrock
    infos.emplace_back(NSID("minecraft:ench_d"), "Ench D", MCE::Bedrock, 2, 2, 1, false,
                       std::unordered_set<NSID>{}, std::unordered_set<NSID>{});
    // max_level=5, All
    infos.emplace_back(NSID("minecraft:ench_e"), "Ench E", MCE::All, 5, 5, 1, false,
                       std::unordered_set<NSID>{}, std::unordered_set<NSID>{});

    EnchantmentRegistry reg(infos);

    // Subset 1: max_level >= 3  →  {ench_a, ench_b, ench_e}
    auto subset1 = reg.create_subset(
        [](const EnchInfo& e) { return e.max_level >= 3; });
    ...
    auto subset2 = subset1.create_subset(
        [](const EnchInfo& e) { return supports_platform(e, MCE::Java); });
...
void test_bulk_insert_200() {
    ...
    for (int i = 0; i < 200; ++i) {
        ...
        auto result = reg.insert(EnchInfo{
            NSID(id_str), "Ench " + std::to_string(i), MCE::All,
            5, 5, 1, false,
            std::unordered_set<NSID>{}, std::unordered_set<NSID>{}
        });
        if (!result.second) {
...
void test_bulk_insert_erase_mixed() {
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        reg.insert(EnchInfo{
            NSID(id_str), "Test " + std::to_string(i), MCE::All,
            5, 5, 1, false,
            std::unordered_set<NSID>{}, std::unordered_set<NSID>{}
        });
    }
    ...
    for (int i = 0; i < 30; ++i) {
        std::string id_str = "minecraft:test_" + std::to_string(i);
        expect(!reg.contains(NSID(id_str)),
               "bulk erase: erased test_" + std::to_string(i) + " not found");
    }
    ...
    for (int i = 30; i < 100; ++i) {
        std::string id_str = "minecraft:test_" + std::to_string(i);
        expect(reg.contains(NSID(id_str)),
               "bulk erase: remaining test_" + std::to_string(i) + " found");
    }
...
void test_bulk_update() {
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        reg.insert(EnchInfo{
            NSID(id_str), "Upd " + std::to_string(i), MCE::All,
            5, 5, 1, false,
            std::unordered_set<NSID>{}, std::unordered_set<NSID>{}
        });
    }
