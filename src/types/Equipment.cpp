...
std::vector<ItemStack> Equipment::filter_items(const ItemCollection &items) const {
    auto view = items | std::views::filter([this](const ItemStack &item) {
                    auto e_view = item.enchantments | std::views::filter([this](const Ench &ench) {
                                      return is_applicable(ench);
                                  });
                    return !e_view.empty();
                });
    return std::vector<ItemStack>{view.begin(), view.end()};
}
