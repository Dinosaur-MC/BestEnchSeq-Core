...
铁砧操作最大接受成本为 **39 级**。超过 39 级时报「过于昂贵！」并拒绝操作。

注意：39 级是**拒绝阈值**，不是封顶——超出阈值的操作无法执行，而非截断至 39 级。
纯更名操作（无附魔合并/修复）有独立的 39 级硬 cap，此处不涉及。

算法搜索中，ForgeEngine 直接返回原始成本不做截断，
外层 OutputFormatter 在输出时检查 `get_peak_level_cost() >= 39` 并显示 ⚠ 警告。
算法自身的 bound 机制（`best_cost`、`initial_bound`）自动剪枝超过当前最优解的分支，
不会因为 ForgeEngine 不封顶而遗漏合法方案。
...
ForgeEngine 对总费用不做截断——若结果 > 39 级，该操作在铁砧上不可行，
搜索算法应通过 bound 机制排除此类步骤。
