...
TEST_CASE("test_serialize_enchinfo") {
    std::unordered_set<NSID> exclusive_set = {NSID("minecraft:smite"), NSID("minecraft:bane_of_arthropods")};
    std::unordered_set<NSID> applicable = {NSID("#minecraft:sword"), NSID("#minecraft:axe")};
    EnchInfo info{NSID("minecraft:sharpness"), "Sharpness", MCE::All, 5, 5, 1, false, exclusive_set, applicable};
    ...
    expect(i2.exclusive_set.count(NSID("minecraft:smite")) == 1, "enchinfo round-trip: exclusive_set contains smite");
...
TEST_CASE("test_enchinfo_platform_key") {
    EnchInfo info{NSID("minecraft:sharpness"), "Sharpness", MCE::Bedrock, 5, 5, 1, false, {}, {}};
    Json j;
    j << info;
    ...
    Json legacy =
        Json::object().set("id", Json(std::string("minecraft:sharpness"))).set("supported_platform", Json(std::string("all")));
    EnchInfo i2;
    ds::ErrorList e;
...
TEST_CASE("test_serialize_enchinfo_min_cost") {
    ...
    EnchInfo info{NSID("minecraft:sharpness"), "Sharpness", MCE::All, 5, 5, 1, false, exclusive_set, applicable, 10, 5};
    ...
    expect(j["min_cost_base"].as<int64_t>() == 10, "min_cost_base serialized value");
    expect(j["min_cost_per_level"].as<int64_t>() == 5, "min_cost_per_level serialized value");
    ...
    EnchInfo plain{NSID("minecraft:smite"), "Smite", MCE::All, 5, 5, 1, false, exclusive_set, applicable};
    Json pj;
    pj << plain;
...
TEST_CASE("test_serialize_equipment") {
    Equipment eq{NSID("minecraft:diamond_sword"), "Diamond Sword", EquipmentTag::sword(), 1561};
    ...
    expect(e2.id == NSID("minecraft:diamond_sword"), "equipment round-trip: id");
...
TEST_CASE("test_serialize_item") {
    ...
    expect(i2.id == NSID("minecraft:diamond_sword"), "item round-trip: id");
    expect(i2.enchantments.size() == 2, "item round-trip: enchantments size");
    expect(i2.enchantments.find(NSID("minecraft:sharpness")) != i2.enchantments.end(), "item round-trip: contains sharpness");
...
TEST_CASE("test_serialize_solution_step") {
    ...
    expect(s2.item_a.id == NSID("minecraft:diamond_sword"), "step round-trip: item_a id");
...
TEST_CASE("test_serialize_solution") {
    ...
    Solution::EnchStep step{Item(NSID("minecraft:diamond_sword"), EnchSet{}, 0, 500),
                            Item(NSID("minecraft:enchanted_book"), step_enchs_a, 0), 15, 60};
    ...
    auto sol = Solution::make(MCE::Java, original, target, available, steps, true, meta);
    ...
    // Core fields
    expect(s2.total_exp_level_cost == sol.total_exp_level_cost, "solution round-trip: total_exp_level_cost");
    expect(s2.total_exp_cost == sol.total_exp_cost, "solution round-trip: total_exp_cost");
    expect(s2.is_success == true, "solution round-trip: is_success");
    expect(s2.platform == MCE::Java, "solution round-trip: platform");

    // Metadata
    expect(s2.metadata.algorithm_name == "dp_merge", "solution round-trip: algorithm_name");
    expect(s2.metadata.algorithm_version == "1.0", "solution round-trip: algorithm_version");
    expect(s2.metadata.mode == AlgorithmMode::direct, "solution round-trip: mode");
    expect(s2.metadata.task_id == 42, "solution round-trip: task_id");

    // Collections
    expect(s2.steps.size() == 1, "solution round-trip: steps count");
    expect(s2.steps[0].exp_level_cost == 15, "solution round-trip: step[0].exp_level_cost");
...
TEST_CASE("test_serialize_equipment_registry") {
    std::vector<Equipment> eqs = {
        Equipment{NSID("minecraft:diamond_sword"), "Diamond Sword", EquipmentTag::sword(), 1561},
        Equipment{NSID("minecraft:diamond_chestplate"), "Diamond Chestplate", EquipmentTag::chestplate(), 528}};
...
TEST_CASE("test_serialize_tag_registry") {
    std::vector<EquipmentTag> tags = {{NSID("#minecraft:sword"), "sword"}, {NSID("#minecraft:axe"), "axe"}};
    ...
    expect(reg2.size() == 2, "equipment tag registry round-trip: size 2");
...
TEST_CASE("test_serialize_ench_registry") {
    ...
    std::vector<EnchInfo> infos = {{NSID("minecraft:sharpness"), "Sharpness", MCE::All, 5, 5, 1, false, excl_sharp, applicable},
                                   {NSID("minecraft:smite"), "Smite", MCE::All, 5, 5, 1, false, excl_smite, applicable}};
    ...
    expect(reg2.size() == 2, "ench registry round-trip: size 2");
    expect(reg2.contains(NSID("minecraft:sharpness")), "ench registry round-trip: contains sharpness");
    expect(reg2.contains(NSID("minecraft:smite")), "ench registry round-trip: contains smite");
...
TEST_CASE("test_serializer_mce_helpers") {
    ...
    for (const auto& tc : cases) {
        ...
        expect(str == tc.expected,
               "mce_to_string(" + std::to_string(static_cast<int>(tc.value)) + "): expected " + std::string(tc.expected));
        auto mce = Serializer::string_to_mce(str);
        expect(mce == tc.value, "string_to_mce round-trip for " + std::string(tc.expected));
    }

    // Case insensitivity of string_to_mce
    expect(Serializer::string_to_mce("JAVA") == MCE::Java, "string_to_mce uppercase JAVA");
...
TEST_CASE("test_to_mc_official_strings") {
    ...
    infos.emplace_back(NSID("minecraft:sharpness"), "Sharpness", MCE::All, 5, 5, 1, false, excl_sharp, sword_applicable);
    infos.emplace_back(NSID("minecraft:smite"), "Smite", MCE::All, 5, 5, 1, false, excl_smite, sword_applicable);

    // Serialize to MC official format
    auto result = EnchSerializer::to_mc_official_strings(infos, tag_reg);

    // Verify map size
    expect(result.size() == 2, "mc_official_strings should have 2 entries");

    // Verify keys exist
    expect(result.find("data/minecraft/enchantment/sharpness.json") != result.end(),
           "mc_official_strings contains sharpness.json key");
    expect(result.find("data/minecraft/enchantment/smite.json") != result.end(), "mc_official_strings contains smite.json key");

    // Verify content strings are non-empty
    const auto& sharp_content = result.at("data/minecraft/enchantment/sharpness.json");
    expect(!sharp_content.empty(), "sharpness content is non-empty");
    const auto& smite_content = result.at("data/minecraft/enchantment/smite.json");
    expect(!smite_content.empty(), "smite content is non-empty");

    // Verify content contains expected fields
    expect(sharp_content.find("anvil_cost") != std::string::npos, "sharpness content contains anvil_cost");
    expect(sharp_content.find("max_level") != std::string::npos, "sharpness content contains max_level");
    expect(sharp_content.find("exclusive_set") != std::string::npos, "sharpness content contains exclusive_set");
    expect(sharp_content.find("supported_items") != std::string::npos, "sharpness content contains supported_items");
    expect(sharp_content.find("minecraft:smite") != std::string::npos, "sharpness exclusive_set contains minecraft:smite");
...
TEST_CASE("test_profile_display_name") {
    ...
    Profile p("my:pack");
    expect_eq(p.display_name(), std::string("my:pack"), "empty display_name falls back to the name key");

    // Set a friendly name; it wins over the key.
    p.set_display_name("My Pack");
    expect_eq(p.display_name(), std::string("My Pack"), "display_name preferred when set");
    ...
    expect_eq(json["display_name"].as<std::string>(), std::string("My Pack"), "display_name serialized value");

    // Round-trip preserves display_name.
    Profile restored;
    json >> restored;
    expect_eq(restored.display_name(), std::string("My Pack"), "display_name preserved after JSON round-trip");
    ...
    expect_eq(clone.display_name(), std::string("My Pack"), "display_name preserved across clone");
