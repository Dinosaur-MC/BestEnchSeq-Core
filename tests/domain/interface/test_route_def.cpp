...
public:
    using Self = DemoCtl;
    int hits = 0;
    Response no_args() { ++hits; return Response::json(200, "OK", "{}"); }
    Response with_req(const HttpRequest&) { ++hits; return Response::json(200, "OK", "{}"); }
    Response with_params(const HttpRequest&, const PathParams&) { ++hits; return Response::json(200, "OK", "{}"); }
    Response with_body(const HttpRequest& r, const Json&) { ++hits; (void)r; return Response::json(200, "OK", "{}"); }
    Response with_both(const HttpRequest& r, const PathParams&, const Json&) { ++hits; (void)r; return Response::json(200, "OK", "{}"); }
};

using Self = DemoCtl;   // BESQ_ROUTE 宏展开引用 Self::H，需在宏使用处可见

constexpr auto kTable = std::array{
    BESQ_ROUTE(Get, "/a", no_args),
    BESQ_ROUTE(Post, "/b", with_req),
    BESQ_ROUTE(Get, "/c/{id}", with_params),
    BESQ_ROUTE(Post, "/d", with_body),
    BESQ_ROUTE(Put, "/e/{id}", with_both),
};
...
void test_invoke_all_forms() {
    DemoCtl ctl;
    HttpRequest req; req.body = "{}";
    PathParams pp; pp.kv.emplace_back("id", "42");
