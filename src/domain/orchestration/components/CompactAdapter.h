...
public:
    ...
    static bool is_supported(const EnchInfo& info, const NSID& item_id, const std::unordered_set<NSID>& item_tags);
    ...
    static bool
    is_applicable(const EnchInfo& info, const NSID& item_id, const std::unordered_set<NSID>& item_tags, MCE platform);
    ...
    ///                                          (tag intersection)
    static algorithm::AlgorithmInput
    apply(const orchestration::SolveSnapshot& snapshot, const SolveRequest& request, const TagResolver& tag_resolver);
    ...
    static std::vector<Solution> recall(const algorithm::AlgorithmOutput& output, const algorithm::AlgorithmInput& input);
