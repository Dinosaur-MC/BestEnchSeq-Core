...
inline void fill_max_levels(
    EnchEnumerator&& enumerate_enchs,
...
inline void precompute_max(
    ...
    const ItemPool& pool,
    const compact::EnchReg& reg,
    std::vector<int16_t>& h_max,
    std::vector<int16_t>& h_dirty)
{
    fill_max_levels(
        [&](auto&& yield) {
            for (auto id : ids)
...
inline int32_t compute_h(
    const std::vector<compact::Ench>& target,
    const compact::EnchReg& reg,
    const std::vector<int16_t>& h_max)
{
    int32_t h = 0;
...
inline int32_t dfs_bound(
    ...
    int32_t best_cost,
    int64_t& node_limit,
    const IForgeEngine& forge_engine,
    const compact::EnchReg& reg,
    const std::vector<compact::Ench>& target,
    std::vector<int16_t>& h_buf,
    std::vector<int16_t>& h_dirty)
{
    if (node_limit <= 0)
        return best_cost;
    ...
    for (auto id : h_dirty) h_buf[id] = 0;
    if (g + h >= best_cost)
        return best_cost;
    ...
    for (size_t i = 0; i < n; ++i) {
        for (size_t j = 0; j < n; ++j) {
            ...
            if (!forge_engine.is_forgeable(items[i], items[j]))
                continue;
            int32_t est = forge_engine.estimate_forge_cost(
                items[i], items[j], reg);
            if (g + est < best_cost)
    ...
    for (const auto& cand : candidates) {
        ...
        int32_t real_cost = forge_engine.forge_into(
            forged, items[cand.j], reg);
        ...
        best_cost = dfs_bound(std::move(child), child_g,
                               best_cost, node_limit,
                               forge_engine, reg, target,
                               h_buf, h_dirty);
        if (node_limit <= 0)
            return best_cost;
    }
