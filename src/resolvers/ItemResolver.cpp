    ...
    const EnchantmentRegistry& ench_reg)
{
    ...
    auto check = [&](const Ench& e, const char* label) {
        ...
        if (!ok) {
            errors.push_back(
                std::string(label) + " '" + info.name_id +
                "' is not applicable to '" +
                target_item.equipment->name_id + "'");
        }
    };

    for (const auto& e : ench_set) check(e, "target_ench");
    for (const auto& e : existing) check(e, "existing_ench");
    return errors;
}
    ...
    const EnchantmentRegistry& ench_reg)
{
    ...
    auto app_errors = check_applicability(target_ench, source_ench,
                                          target_item, ench_reg);
    if (!app_errors.empty()) {
        std::string msg;
