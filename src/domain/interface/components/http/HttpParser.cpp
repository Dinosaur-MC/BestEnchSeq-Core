...
// 头名出现次数（大小写不敏感）。
size_t count_header(const std::vector<std::pair<std::string, std::string>>& headers, std::string_view name) {
    size_t n = 0;
...
ParseResult parse_incremental(const std::string& buf, size_t& consumed, HttpRequest& out) {
    ...
    out.keep_alive = version == "HTTP/1.1" ? !header_value_is(out.headers, "Connection", "close")
