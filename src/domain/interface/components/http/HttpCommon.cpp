...
std::string HttpResponse::to_bytes(bool keep_alive) const {
    ...
    for (const auto& kv : headers)
        if (ieq(kv.first, "Content-Length")) {
...
HttpResponse HttpResponse::no_content() {
    return HttpResponse{204, "No Content", "", {}, "", false};
}
HttpResponse HttpResponse::method_not_allowed(const std::string& allow) {
    HttpResponse r =
        json(405, "Method Not Allowed", R"({"ok":false,"error":{"code":"METHOD_NOT_ALLOWED","message":"method not allowed"}})");
