...
/// `http_host`/`http_port`/`http_workers`/`memory_mb`/`sandbox_enabled` and the
...
/// the five writable fields to <cwd>/config.json (best-effort; the service
/// reloads them at startup — see AppConfig).
///
/// `effective_port`（可选）指向实际绑定端口，由 WebModule 在 server.start()
/// 成功后注入（配置 0 = OS 自动分配，真实端口 >0）：GET 的 http_port 用它
