...
void test_resolve_basic() {
    ...
    auto result = RawTypeAdapter::resolve(enchants, equipments);

    expect(result.cat_reg.get_id("sword") >= 0, "sword category created");
    expect(result.cat_reg.get_id("axe") >= 0, "axe category created");
    expect(result.eq_reg.size() == 2, "two equipment registered");
    expect(result.ench_reg.size() == 1, "one enchantment registered");

    const auto& instances = result.ench_reg.get_instances();
...
void test_resolve_empty() {
    auto result = RawTypeAdapter::resolve({}, {});

    expect(result.ench_reg.size() == 0, "no enchantments");
    expect(result.eq_reg.size() == 0, "no equipment");

    // Category registry always has builtin categories (15: "any" + 14 specific).
    expect(result.cat_reg.size() == 15,
           "category registry has 15 builtin categories");

    // Builtin "any" category is always present.
    expect(result.cat_reg.get_id("any") >= 0, "builtin 'any' category exists");
...
void test_resolve_category_dedup() {
    ...
    auto result = RawTypeAdapter::resolve({}, equipments);

    int32_t sword_id = result.cat_reg.get_id("sword");
    expect(sword_id >= 0, "sword category exists");

    // Both equipment should reference the same category id
    const auto& instances = result.eq_reg.get_instances();
...
void test_resolve_enchantment_exclusive_set() {
    ...
    expect(result.ench_reg.size() == 2, "two enchantments registered");
    expect(result.ench_reg.is_incompatible(0, 1),
           "sharpness and bane are incompatible");
    expect(result.ench_reg.is_incompatible(1, 0),
