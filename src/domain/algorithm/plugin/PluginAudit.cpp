...
struct MappedFile {
    ...
    bool map(const std::string &path) {
        ...
        if (size == 0) { CloseHandle(hFile); hFile = INVALID_HANDLE_VALUE; return false; }
        ...
        if (!data) { CloseHandle(hMap); hMap = nullptr; CloseHandle(hFile); hFile = INVALID_HANDLE_VALUE; return false; }
        return true;
    }
...
static PluginAuditReport audit_elf(const std::string &path) {
    ...
    if (!dynsym_sec || !dynstr_sec) {
        LOG_WARN("[Audit] '%s' has no dynamic symbol table", path.c_str());
        return report;
    }
    ...
    for (size_t i = 0; i < sym_count; ++i) {
        ...
        if (syms[i].st_shndx == SHN_UNDEF) {
            ...
            if (type != STT_FUNC && type != STT_NOTYPE)
                continue;
            if (is_runtime_sym(sym_name))
                continue;
            if (is_red_flag(sym_name))
                report.dangerous_imports.emplace_back(sym_name);
        } else if (bind == STB_GLOBAL) {
            // — Exported (defined, global) symbol —
            if (type != STT_FUNC && type != STT_NOTYPE)
                continue;
            if (is_besq_export(sym_name))
                continue;
            report.extra_exports.emplace_back(sym_name);
        }
    }

    // ── Parse .dynamic → DT_NEEDED libraries ──────────────────────
    if (dynamic_sec && dynstr_dyn &&
        dynamic_sec->sh_offset + dynamic_sec->sh_size <= fsize &&
        ...
        for (size_t i = 0; i < n_dyn; ++i) {
            if (dyn[i].d_tag == DT_NEEDED) {
                ...
                if (off < dynstr_dyn->sh_size)
                    report.linked_libraries.emplace_back(dynstr_base + off);
            }
        }
    }
...
static DWORD rva_to_offset(const IMAGE_NT_HEADERS64 *nt,
                           const uint8_t *base, DWORD rva) {
    const IMAGE_SECTION_HEADER *sec = IMAGE_FIRST_SECTION(nt);
    for (WORD i = 0; i < nt->FileHeader.NumberOfSections; ++i) {
        const DWORD va  = sec[i].VirtualAddress;
        const DWORD sz  = sec[i].SizeOfRawData;
        if (rva >= va && rva < va + sz)
            return rva - va + sec[i].PointerToRawData;
    }
    return 0;
}
...
static PluginAuditReport audit_pe(const std::string &path) {
    ...
    if (nt_offset < 0 ||
        static_cast<DWORD>(nt_offset) + sizeof(IMAGE_NT_HEADERS64) > fsize) {
    ...
    if (export_base && export_size >= sizeof(IMAGE_EXPORT_DIRECTORY)) {
        ...
        if (exp->NumberOfNames > 0 && exp->AddressOfNames != 0) {
            ...
            if (names_off > 0) {
                ...
                const DWORD limit = (max_names < exp->NumberOfNames)
                                        ? static_cast<DWORD>(max_names)
                                        : exp->NumberOfNames;
    ...
    if (import_base && import_size >= sizeof(IMAGE_IMPORT_DESCRIPTOR)) {
        ...
        for (size_t d = 0; d < max_descs; ++d) {
            ...
            report.linked_libraries.emplace_back(dll_name);

            // Thunk table
            const DWORD thunk_rva = imp_desc[d].OriginalFirstThunk
                                        ? imp_desc[d].OriginalFirstThunk
                                        : imp_desc[d].FirstThunk;
            ...
            while (thunk->u1.AddressOfData != 0) {
                if (!IMAGE_SNAP_BY_ORDINAL64(thunk->u1.Ordinal)) {
                    const DWORD iname_off = rva_to_offset(nt, base,
                        static_cast<DWORD>(thunk->u1.AddressOfData));
                    if (iname_off != 0 &&
                        iname_off + sizeof(IMAGE_IMPORT_BY_NAME) <= fsize) {
                        const auto *import_by_name =
                            reinterpret_cast<const IMAGE_IMPORT_BY_NAME *>(base + iname_off);
                        const char *func_name = import_by_name->Name;
                        const size_t avail = fsize - iname_off - 2;
                        const size_t flen = strnlen(func_name, avail);
                        if (flen > 0 && flen < avail && is_red_flag(func_name)) {
                            report.dangerous_imports.emplace_back(
                                std::string(dll_name) + "!" + std::string(func_name, flen));
                        }
                    }
                }
                ++thunk;
