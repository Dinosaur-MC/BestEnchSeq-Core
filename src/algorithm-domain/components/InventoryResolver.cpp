    ...
    const EquipmentRegistry& eq_reg)
{
    ...
    if (root.type() != JsonType::Object) return result;

    Json::Value root_val = root.get_value();
    if (!std::holds_alternative<Json::Object>(root_val)) return result;
    Json::Object root_obj = std::get<Json::Object>(root_val);

    auto items_it = root_obj.find("items");
    if (items_it == root_obj.end()) return result;

    Json::Value items_var = items_it->second.get_value();
    if (!std::holds_alternative<Json::Array>(items_var)) return result;
        ...
        if (type == "book") {
            ...
            if (eq_id >= 0) {
                ...
                auto& item = result.items.emplace_back(
                    equip, ench_set, prior_penalty, durability);
                item.priority = priority;
            } else {
                result.warnings.push_back(
                    "unknown equipment '" + equip_id + "', treating as book");
                auto& item = result.items.emplace_back(ench_set, prior_penalty);
                item.priority = priority;
            }
        } else {
            result.warnings.push_back(
                "unknown item type '" + type + "', skipping");
        }
    }
