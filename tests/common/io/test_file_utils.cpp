...
TEST_CASE("test_file_utils") {
    ...
    if (entries.size() == 4) {
        expect(entries[0].is_dir && entries[0].name == "sub", "entry 0 = sub directory first");
        expect(entries[1].is_dir && entries[1].name == "zdir", "entry 1 = zdir directory second");
        expect(!entries[2].is_dir && entries[2].name == "a.txt", "entry 2 = a.txt after directories");
        expect(!entries[3].is_dir && entries[3].name == "b.txt", "entry 3 = b.txt name-sorted");
    ...
    expect(file_utils::list_directory(base / "nope").empty(), "missing directory yields empty list");
    expect(file_utils::list_directory(base / "a.txt").empty(), "a file yields empty list");
