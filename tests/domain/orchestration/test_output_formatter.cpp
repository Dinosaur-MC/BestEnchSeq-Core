...
Profile profile_from_fx(const TestFixture& fx) {
    ...
    for (const auto& tag : fx.categories)
        profile.add_tag(tag);
    for (const auto& eq : fx.equipment)
        profile.add_equipment(eq);
    for (const auto& ench : fx.enchants)
...
TEST_CASE("test_verbose_item_format") {
    ...
    expect(output.find("output.item.free") != std::string::npos || output.find("(free)") != std::string::npos,
           "verbose output should show free indicator for empty items with ppn=0");
    expect(output.find("enchanted_book") != std::string::npos, "verbose output should show 'enchanted_book' for books");
...
TEST_CASE("test_format_json_real_equipment") {
    ...
    auto json_str = OutputFormatter::format_json({solution}, profile, AlgorithmMode::direct, true, "dp_merge", 42);
    Json root = Json::parse(json_str);

    // Root metadata aligned with the C ABI (besq_solve).
    expect(root["success"].as<bool>() == true, "format_json root: success");
    expect(root["algorithm"].as<std::string>() == "dp_merge", "format_json root: algorithm");
    expect(root["computation_time_ms"].as<int64_t>() == 42, "format_json root: computation_time_ms");
    ...
    expect(target["equipment"]["id"].as<std::string>() == "minecraft:diamond_sword", "format_json: equipment id");
    expect(target["equipment"]["max_durability"].as<int32_t>() == 1561, "format_json: real max_durability");
    expect(target["equipment"]["category"].as<std::string>() == "sword", "format_json: real category short name");
...
TEST_CASE("test_format_json_step_result") {
    ...
    expect(res["is_book"].as<bool>() == false, "format_json: step result is forged equipment, not a book");
    expect(res["equipment"]["id"].as<std::string>() == "minecraft:diamond_chestplate",
           "format_json: step result keeps the target equipment id");
    expect(res["enchantments"].as_array().size() == 1, "format_json: step result carries the forged enchantments");
...
TEST_CASE("test_format_compact_platform_raw") {
    ...
    expect(zh.find("#PLATFORM=Java\n") != std::string::npos, "compact zh_CN: emits #PLATFORM=Java (raw enum name)");
    expect(zh.find("Java版") == std::string::npos, "compact zh_CN: must NOT leak localized platform name");

    // en_US: same raw name.
    lm.select("en_US");
    auto en = OutputFormatter::format_compact({solution}, profile, AlgorithmMode::direct);
    expect(en.find("#PLATFORM=Java\n") != std::string::npos, "compact en_US: emits #PLATFORM=Java (raw enum name)");
...
TEST_CASE("test_json_roundtrip") {
    ...
    expect(parsed[0].steps[0].result.id == NSID("minecraft:diamond_sword"), "round-trip: step result item preserved");
    expect_eq(parsed[0].steps[0].result.prior_penalty, 1, "round-trip: step result prior penalty preserved");
    expect_eq(parsed[0].total_exp_level_cost, 5, "round-trip: total level cost");
    expect(parsed[0].target_item.id == NSID("minecraft:diamond_sword"), "round-trip: target equipment preserved");
...
Json reference_item_json(const Item& item, const TagRegistry& cat_reg, const EquipmentRegistry& eq_reg) {
    Json::Object obj;
    ...
    if (!item.is_book()) {
        ...
        if (auto eq_it = eq_reg.find(item.id); eq_it != eq_reg.end()) {
            ...
            if (auto tag_it = cat_reg.find(eq_it->category); tag_it != cat_reg.end()) {
                category_name = tag_it->name;
            } else {
                category_name = business::parser_detail::category_short_name(eq_it->category);
...
Json reference_solution_json(const Solution& sol, int32_t rank, const TagRegistry& cat_reg, const EquipmentRegistry& eq_reg) {
    Json::Object s;
    s["rank"] = Json(rank);
    switch (sol.platform) {
    case MCE::Java:
        s["platform"] = Json("Java");
        break;
    case MCE::Bedrock:
        s["platform"] = Json("Bedrock");
        break;
    case MCE::All:
        s["platform"] = Json("All");
        break;
    default:
        s["platform"] = Json("None");
        break;
    }
...
TEST_CASE("test_format_json_schema_encode") {
    ...
    solution.metadata.created_at = std::chrono::system_clock::time_point(std::chrono::seconds(1700000000));
    solution.metadata.computation_time = std::chrono::milliseconds(42);
    ...
    const auto json_str = OutputFormatter::format_json({solution}, profile, AlgorithmMode::direct, true, "dp_merge", 42);
    Json root = Json::parse(json_str);

    // 1) schema_version bumped to 1.1
    expect(root["schema_version"].as<std::string>() == "1.1", "schema encode: schema_version 1.1");
    ...
    Json expected =
        Json::object()
            ...
            .set("solutions", Json(Json::Array{reference_solution_json(solution, 1, profile.tags(), profile.eq())}));
    ...
    expect(d.metadata.algorithm_name == "dp_merge", "schema decode: metadata.algorithm_name");
    ...
    expect(d_step.result.equipment->id == "minecraft:diamond_chestplate", "schema decode: result equipment id");
    expect_eq(d_step.result.enchantments.size(), 1u, "schema decode: result enchantments");
    expect(d.target_item.equipment.has_value(), "schema decode: target equipment");
    expect_eq(d.target_item.equipment->max_durability, 528, "schema decode: target max_durability");
    expect(d.target_item.equipment->category == "chestplate", "schema decode: target category");
...
TEST_CASE("test_build_json_root_schema") {
    Json root = OutputFormatter::build_json_root(AlgorithmMode::inventory, false, "bb_dp", 1234);
...
template <typename Tuple> static std::vector<std::string> schema_field_names(const Tuple& t) {
...
TEST_CASE("test_root_meta_fields_shared") {
    ...
    expect(root_prefix == meta, "root schema prefix must match RootMetaSchema fields (shared tuple)");
